...
#define _TDMB_MAX_PCBER_MAX_ 200000
...
void stream_upload (uint32_t size, BUFF * q, uint32_t type, int32_t moduleidx, int32_t subchid)
{
  ...
  if(!gp_MSCFICData || !gp_MSCFICData[moduleidx])
  {
    PRINTF_LV_0("[%s]   gp_MSCFICData in NULL \n",__func__);
    return;
  }  
  ...
  if (q->wptr > q->rptr)
  {
    ...
    TC317X_IO_memcpy (gp_MSCFICData[moduleidx], &q->p[q->rptr], size);
  }
  else
  {
...
void TDMB_Alloc (int32_t moduleidx, uint32_t size)
{
  ...
  if (!gp_TDMB_Stream)
  {
    gp_TDMB_Stream = (uint8_t **) TC317X_IO_malloc (maxbb * sizeof (uint8_t *));
  ...
  if (!gp_MSCFICData)
  {
    gp_MSCFICData = (uint8_t **) TC317X_IO_malloc (maxbb * sizeof (uint8_t *));
...
uint32_t Cal_TDMB_PCBER (TDMB_StatusData * pDMBStatData)
{
  ...
  else if (PCBER > _TDMB_MAX_PCBER_MAX_)
    PCBER = _TDMB_MAX_PCBER_MAX_;
...
uint32_t Cal_TDMB_VITERBIBER (TDMB_StatusData * pDMBStatData, uint32_t OldVITERBIBER)
{
  ...
  overCnt = (pDMBStatData->RS_OVER_CNT - pDMBStatData->OLD_RS_OVER_CNT);

  errCnt = pDMBStatData->RS_ERR_CNT - pDMBStatData->OLD_RS_ERR_CNT;
  temp = pDMBStatData->RS_PKT_CNT - pDMBStatData->OLD_RS_PKT_CNT;
  under = temp * 204 * 8;

  //over = errCnt * SCALE_FACTOR;
  over = (overCnt*(8*8) + errCnt) * SCALE_FACTOR;
...
void GetTDMBStream (int32_t moduleidx, uint32_t interrupt_stat)
{
  ...
  if(!gp_TDMB_Stream || !gp_TDMB_Stream[moduleidx])
  {
    PRINTF_LV_0("[%s]   gp_TDMB_Stream in NULL \n", __func__);
    return;
  }  
...
void TDMB_Reset_MonitorValue(int32_t moduleidx)
{
  ...
  gtTDMBCtrl[moduleidx].MonStat.PCBERset.PCBER = _TDMB_MAX_PCBER_MAX_;//_TDMB_MAX_PCBER_*SCALE_FACTOR;
  gtTDMBCtrl[moduleidx].MonStat.PCBERset.Moving_AVG =
