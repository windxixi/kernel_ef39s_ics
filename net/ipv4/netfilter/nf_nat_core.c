...
int nf_nat_protocol_register(const struct nf_nat_protocol *proto)
{
	...
	rcu_assign_pointer_nonull(nf_nat_protos[proto->protonum], proto);
 out:
	spin_unlock_bh(&nf_nat_lock);
	return ret;
}
...
static void __exit nf_nat_cleanup(void)
{
	...
	nf_ct_extend_unregister(&nat_extend);
	rcu_assign_pointer_nonull(nf_nat_seq_adjust_hook, NULL);
	rcu_assign_pointer_nonull(nfnetlink_parse_nat_setup_hook, NULL);
	rcu_assign_pointer_nonull(nf_ct_nat_offset, NULL);
	synchronize_net();
}
